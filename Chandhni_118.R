...
getwd()
setwd("C:\\Users\\admin1\\Documents\\PGP2\\AMMA\\Assignment 2\\AMMA - 2-20170828T183349Z-001\\AMMA - 2") 
...
titanic_train_mean_Chandhni <- titanic_train_start
titanic_train_mean_Chandhni2 <- titanic_train_start
titanic_train_mean_Chandhni$Age[is.na(titanic_train_mean_Chandhni$Age)] = mean(titanic_train_mean_Chandhni$Age, na.rm = TRUE)
titanic_train_mean_Chandhni2$Age[is.na(titanic_train_mean_Chandhni2$Age)] = mean(titanic_train_mean_Chandhni2$Age, na.rm = TRUE)
...
full.model.titanic.mean <- glm(formula = Survived ~ Pclass + Sex + SibSp + Parch + Fare + Age,
                               data=titanic_train_mean_Chandhni, family = binomial) #family = binomial implies that the type of regression is logistic

#lm
fit.train.mean <- lm(formula = Survived ~ Pclass + Sex + SibSp + Parch + Fare + Age,
                     data=titanic_train_mean_Chandhni2) #family = binomial implies that the type of regression is logistic
...
titanic_train_mean_Chandhni$prob = predict(full.model.titanic.mean, type=c("response"))
titanic_train_mean_Chandhni$Survived.pred = ifelse(titanic_train_mean_Chandhni$prob>=.5,'pred_yes','pred_no')
table(titanic_train_mean_Chandhni$Survived.pred,titanic_train_mean_Chandhni$Survived)

#Testing performance on test set
nrow(titanic_test)
nrow(titanic_test2_mean_Chandhni)
titanic_test2_mean_Chandhni <- titanic_test_start

#imputation by replacing NAs by means in the test set
titanic_test2_mean_Chandhni$Age[is.na(titanic_test2_mean_Chandhni$Age)] = mean(titanic_test2_mean_Chandhni$Age, na.rm = TRUE)

titanic_test2_mean_Chandhni$prob = predict(full.model.titanic.mean, newdata=titanic_test2_mean_Chandhni, type=c("response"))
titanic_test2_mean_Chandhni$Survived.pred = ifelse(titanic_test2_mean_Chandhni$prob>=.5,'pred_yes','pred_no')
table(titanic_test2_mean_Chandhni$Survived.pred,titanic_test2_mean_Chandhni$Survived)
...
Kfoldobj <- Kfold_func(titanic_train_mean_Chandhni,Survived ~ Pclass + Sex + SibSp + Age,binomial,10)

#Calling the Mean Squared Error function on the training set here
MSE_Train <-MeanSquareError_func(titanic_train_mean_Chandhni,Survived ~ Pclass + Sex + SibSp + Age)

#confusion matrix on training set
table(titanic_train_mean_Chandhni$Survived,round(Kfoldobj$cvhat))
print("Estimate of Accuracy")
print(Kfoldobj$acc.cv)

#Performing KFold CV on test set by calling the KFOLD CV function here
Kfoldobj.test <- Kfold_func(titanic_test2_mean_Chandhni,Survived ~ Pclass + Sex + SibSp + Age,binomial,10)

#Calling the Mean Squared Error function on the test set here
MSE_Test <-MeanSquareError_func(titanic_test2_mean_Chandhni,Survived ~ Pclass + Sex + SibSp + Age)

#Confusion matrix on test set
table(titanic_test2_mean_Chandhni$Survived,round(Kfoldobj.test$cvhat))
print("Estimate of Accuracy")
